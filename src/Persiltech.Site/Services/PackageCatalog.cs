...
public sealed class PackageCatalog : IPackageCatalog
{
    ...
    private static readonly NuGetPackage Membership = new(
        ...
        Releases:
        [
            new PackageRelease("0.9.0", "Aviso por correo cuando una cuenta queda bloqueada por intentos fallidos: IMembershipEmailSender estrena SendAccountLockedAsync. La respuesta del inicio de sesión sigue sin distinguir contraseña incorrecta de cuenta bloqueada —distinguirlo confirmaría que ese correo tiene cuenta—, así que la información viaja solo al buzón, donde además es la primera señal de que alguien está probando contraseñas. El aviso no lleva testigo: con uno, fallar la contraseña de alguien bastaría para mandarle un enlace de reinicio que no ha pedido. Rompe a quien implemente IMembershipEmailSender por su cuenta, porque la interfaz gana un método."),
    ...
    private static readonly NuGetPackage MembershipEmail = new(
        ...
        Releases:
        [
            new PackageRelease("0.3.0", "Nueva plantilla AccountLocked para el aviso de bloqueo que estrena Persiltech.Membership 0.9.0. Es la única sin enlace de vuelta: no lleva testigo, así que no hay pantalla a la que dirigir a nadie, y dispone de {{LockoutMinutes}} para decir cuánto dura el bloqueo en lugar de a qué hora termina, que dependería de la zona horaria de quien lo lee. Las cuatro plantillas se rediseñan: el botón gana su variante VML para Outlook de escritorio, los mensajes dejan de quedar ilegibles en modo oscuro, en móvil el botón ocupa el ancho y el cuerpo sube a 16 px, y el pie deja de escribir un separador colgando y un mailto vacío cuando no hay SupportEmail. Dos opciones nuevas: OnPrimaryColor, porque el blanco no se lee sobre una marca clara, y Language, porque el idioma estaba fijado a español dentro de la maqueta."),
